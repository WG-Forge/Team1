...
namespace RailGraph {
Graph::Graph(std::string name, size_t id, const std::vector<Point> &points,
...
void Graph::SetName(const std::string &name_) { this->name = name_; }

void Graph::SetId(size_t id) { idx = id; }
void Graph::SetPoints(const std::vector<Point> &points_) { points = points_; }
void Graph::SetLines(const std::vector<Line> &lines_) { lines = lines_; }
...
Graph::Point::Point(size_t idx_, std::optional<size_t> postIdx_)
    : postIdx(std::move(postIdx_)), idx(idx_) {}
